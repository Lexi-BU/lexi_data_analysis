...
for plot_start_time, plot_end_time in zip(plot_start_time_list, plot_end_time_list):
    ...
    ax[0].set_ylabel("All Counts")
    # ax[0].set_yscale("log")
    twin_ax_1 = ax[0].twinx()
    twin_ax_1.scatter(
        selected_merged_df.index,
    ...
    ax[0].grid(color="c", linestyle="--", linewidth=0.5, alpha=0.5)
    ...
    ax[1].grid(color="c", linestyle="--", linewidth=0.5, alpha=0.5)

    # Set the spine color to match the line color
    twin_ax.spines["left"].set_color("c")
    twin_ax.spines["right"].set_color("m")
    ax[1].tick_params(axis="y", colors="c")
    ...
    # Save the figure
    fig.savefig(
        f"../figures/lexi_data_{plot_start_time}_to_{plot_end_time}.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
