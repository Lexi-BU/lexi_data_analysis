    ...
    line_profiles.append(
        {
            "x": scaled_x_centers,
            "y": hist_slice,
            "name": f"{scaled_y_list[i]:.1f}°",
            "y_value": y_scaled,
        }
    )

# Line profiles (right subplot)
for i, profile in enumerate(line_profiles):
    fig.add_trace(
        go.Scatter(
            ...
            showlegend=True,
            hoverinfo="skip",
            customdata=[i],  # index for identification
...
# Invisible horizontal lines (left subplot)
for i, profile in enumerate(line_profiles):
    fig.add_trace(
        go.Scatter(
            x=[0, 9],
            y=[profile["y_value"], profile["y_value"]],
            mode="lines",
            line=dict(width=0),
            hoverinfo="text",
            text=f"Azimuth: {profile['name']}",
            showlegend=False,
            customdata=[i],  # index for identification
        ),
        row=1,
...
fig.update_layout(
    ...
    updatemenus=[
        dict(
            ...
            showactive=False,
            buttons=[
                dict(label="Reset", method="restyle", args=["opacity", [0.1] * len(line_profiles)])
            ],
            x=1.05,
            xanchor="left",
            y=1.15,
    ...
    annotations=[
        dict(
            ...
            y=-0.2,
            showarrow=False,
            text="Hover over horizontal lines to highlight corresponding profiles",
            xref="paper",
            yref="paper",
        )
    ],
...
window.addEventListener('DOMContentLoaded', function() {{
    ...
    var num_profiles = {len(line_profiles)};
    var profile_traces_start = 1;  // index where your line profiles start in fig.data

    plot_element.on('plotly_hover', function(eventData) {{
        // eventData.points contains hovered points; find profile index from customdata
        var profileIndex = eventData.points[0].customdata;
        if (profileIndex !== undefined && profileIndex >= 0 && profileIndex < num_profiles) {{
            ...
            for (var i = 0; i < num_profiles; i++) {{
                if (i === profileIndex) {{
                    ...
                    opacities.push(0.1);
                    widths.push(2);
                    colors.push('rgba(0,0,255,0.1)');
                }}
            }}

            var trace_indices = [];
            for (var i = 0; i < num_profiles; i++) {{
                trace_indices.push(profile_traces_start + i);
            }}

            Plotly.restyle(plot_element, {{
                'opacity': [opacities],
                'line.width': [widths],
                'line.color': [colors]
            }}, trace_indices);
    ...
    plot_element.on('plotly_unhover', function() {{
        var trace_indices = [];
        for (var i = 0; i < num_profiles; i++) {{
            trace_indices.push(profile_traces_start + i);
        }}
        Plotly.restyle(plot_element, {{
            'opacity': [Array(num_profiles).fill(0.1)],
            'line.width': [Array(num_profiles).fill(2)],
            'line.color': [Array(num_profiles).fill('rgba(0,0,255,0.1)')]
        }}, trace_indices);
    }});
}});
"""
