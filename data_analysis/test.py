...
for y_scaled in scaled_y_list:
    ax1.axhline(y=y_scaled, color="red", linestyle=":", alpha=0.5)
...
for i, y in enumerate(y_list):
    ...
    ax2.plot(scaled_x_centers, hist_slice, label=f"{scaled_y_list[i]:.1f}°", marker=".")
...
fig_file_name = (
    ...
    f"{input_dict['end_time'].replace(':', '').replace('-', '').replace('T', '_')}_rot_angle_{rot_angle}_{scaled_y_list[0]:.0f}_{scaled_y_list[-1]:.0f}_{len(scaled_y_list)}.png"
)
